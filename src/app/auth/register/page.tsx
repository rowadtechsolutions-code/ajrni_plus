      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8">
          ...
          <div className="grid grid-cols-2 gap-2 mb-6">
            <button type="button" onClick={() => { setRole("CUSTOMER"); setValue("role", "CUSTOMER") }} className={cn("p-3 rounded-xl border-2 text-sm font-medium transition-all", role === "CUSTOMER" ? "border-secondary bg-secondary/5 text-secondary" : "border-border text-muted-foreground")}>
              <User className="w-5 h-5 mx-auto mb-1" />{t("auth.as_customer")}
            </button>
            <button type="button" onClick={() => { setRole("OFFICE"); setValue("role", "OFFICE") }} className={cn("p-3 rounded-xl border-2 text-sm font-medium transition-all", role === "OFFICE" ? "border-secondary bg-secondary/5 text-secondary" : "border-border text-muted-foreground")}>
