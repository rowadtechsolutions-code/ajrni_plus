...
import { Plus, Pencil, Trash2, AlertTriangle, Loader2, ArrowLeft } from "lucide-react"
      ...
      {showAddModal ? (
        <div className="fixed inset-0 z-[9999] bg-white flex flex-col">
          <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100 shrink-0">
            <button onClick={() => setShowAddModal(false)} className="p-1 -ml-1 rounded-lg hover:bg-muted transition-colors" aria-label={locale === "ar" ? "رجوع" : "Back"}>
              <ArrowLeft className={`w-5 h-5 ${locale === "ar" ? "rotate-180" : ""}`} />
            </button>
            <h2 className="text-lg font-bold text-primary">
              {editingCar ? (locale === "ar" ? "تعديل السيارة" : "Edit Car") : (locale === "ar" ? "إضافة سيارة" : "Add Car")}
            </h2>
          </div>
          <div className="flex-1 overflow-y-auto px-4 md:px-6 pb-28 md:pb-8">
